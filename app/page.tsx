  ...
  return (
    <main>
      ...
      <section className="section about-grid" id="nosotros">
        ...
        <div className="about-visual"><div className="photo-crop about-photo"><img src={site.about_image || "/images/vial2.png"} alt="Conificación de una vía urbana" loading="lazy" decoding="async" /><div className="about-quote"><span className="about-quote-mark" aria-hidden="true">“</span><p><strong>Un entorno más seguro,</strong><span>es un mejor futuro para todos.</span></p></div></div></div>
      ...
      <section className="coverage" id="cobertura"><div className="coverage-copy"><div className="eyebrow light">Cobertura</div><h2>Concepción y toda la <span>Región del Biobío</span></h2><p>Prestamos nuestros servicios en {site.coverage}. Nos desplazamos donde tu proyecto lo necesite.</p><button className="button button-outline" type="button" onClick={() => setQuoteOpen(true)}>Ver cobertura completa <ChevronRight size={18} /></button></div><div className="coverage-region" role="img" aria-label="Cobertura desde Concepción hacia la Región del Biobío"><span className="coverage-region-ring coverage-region-ring-outer" /><span className="coverage-region-ring coverage-region-ring-inner" /><span className="coverage-region-route coverage-region-route-one" /><span className="coverage-region-route coverage-region-route-two" /><span className="coverage-region-route coverage-region-route-three" /><span className="coverage-region-pin"><MapPin aria-hidden="true" /></span><span className="coverage-region-label">CONCEPCIÓN<small>REGIÓN DEL BIOBÍO</small></span></div><div className="coverage-locations"><span>Comunas donde trabajamos</span><ul>{(projectLocations.length ? projectLocations : ["Concepción", "Talcahuano", "San Pedro de la Paz", "Chiguayante", "Hualpén"]).map(location => <li key={location}><CheckCircle2 /> {location}</li>)}<li><CheckCircle2 /> Y más comunas</li></ul></div></section>
      ...
      {selectedProject !== null && projects[selectedProject] && <ProjectViewer project={projects[selectedProject]} image={projects[selectedProject].image_url || serviceImages[selectedProject % serviceImages.length]} onClose={() => setSelectedProject(null)} onQuote={() => { setSelectedProject(null); setQuoteOpen(true); }} />}
    </main>
  );
}
