...
export async function GET(request: Request) {
  ...
  try {
    const [quotes, projects, services, images, recent, settings] = await Promise.all([
    ...
    return NextResponse.json({ counts:{newQuotes:quotes[0]?.count||0,projects:projects[0]?.count||0,services:services[0]?.count||0,images:images[0]?.count||0}, recent, settings:Object.fromEntries(settings.map((row:any)=>[row.key,row.value])) }, { headers:{"Cache-Control":"no-store"} });
