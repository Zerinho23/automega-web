...
type Item = { id: string; title: string; description: string; visible: boolean; sort_order: number; image_url?: string; storage_path?: string; location?: string; status?: string; name?: string; email?: string; city?: string; service_name?: string; message?: string; created_at?: string };
...
export default function AdminWorkspace({ section }: { section: string }) {
  ...
  useEffect(() => {
    if (!supabase) { setLoading(true); fetch(`/api/admin/data?section=${section}`,{cache:"no-store"}).then(r=>r.ok?r.json():null).then((payload:any)=>{if(payload?.items)setItems(payload.items.map((row:any)=>section==="images"?({...row,title:row.alt_text||row.file_name,description:row.file_name,image_url:row.public_url}):row));if(payload?.settings)setSettings(current=>({...current,...payload.settings}));}).catch(()=>toast.error("No fue posible actualizar los datos")).finally(()=>setLoading(false)); return; }
    if (table) {
  ...
  async function removeItem(id:string) { if (!id.startsWith("draft-")) { if (supabase) { const item=items.find(entry=>entry.id===id); if(section==="images"&&item?.storage_path)await supabase.storage.from("site-images").remove([item.storage_path]); const { error }=await supabase.from(table || "site_images").delete().eq("id",id); if(error){toast.error(error.message);return;} } else { const response=await fetch("/api/admin/data",{method:"DELETE",headers:{"Content-Type":"application/json"},body:JSON.stringify({section,id})}); if(!response.ok){toast.error("No fue posible eliminar el elemento");return;} } } setItems(list=>list.filter(i=>i.id!==id)); toast.success("Elemento eliminado"); }
  async function saveItems() {
    ...
    if (supabase && table) {
      ...
      const response=await fetch("/api/admin/data",{method:"PATCH",headers:{"Content-Type":"application/json"},body:JSON.stringify({section,items})});
      if(!response.ok){toast.error("No fue posible guardar los cambios");setLoading(false);return;}
      const fresh:any=await fetch(`/api/admin/data?section=${section}`,{cache:"no-store"}).then(r=>r.json());
      if(fresh?.items)setItems(fresh.items);
    }
    setLoading(false); toast.success("Cambios guardados en Neon");
  }
  async function saveSettings() {
    setLoading(true); if(supabase){const payload=Object.entries(settings).map(([key,value])=>({key,value}));const {error}=await supabase.from("site_settings").upsert(payload,{onConflict:"key"});if(error){toast.error(error.message);setLoading(false);return;}} else { const response=await fetch("/api/admin/data",{method:"PATCH",headers:{"Content-Type":"application/json"},body:JSON.stringify({section:"settings",settings})}); if(!response.ok){toast.error("No fue posible guardar la configuración");setLoading(false);return;} } setLoading(false);toast.success("Configuración guardada");
  }
  async function uploadImage(file?:File, target="gallery") {
    ...
    try {
      ...
      if(target!=="gallery" && !target.startsWith("project:")){const key=target==="hero"?"hero_image":target==="logo"?"logo_url":"about_image";setSettings(current=>{const next={...current,[key]:imageUrl};try{localStorage.setItem("automega_site_settings",JSON.stringify(next));}catch{}return next;});}
  ...
  return <>
    ...
    {section === "quotes" && <section className="admin-card"><div className="table-wrap"><table><thead><tr><th>Cliente</th><th>Correo</th><th>Servicio</th><th>Comuna</th><th>Estado</th></tr></thead><tbody>{items.length===0?<tr><td colSpan={5} className="empty-table">No hay solicitudes registradas.</td></tr>:items.map(item=><tr key={item.id}><td><strong>{item.name}</strong><small className="cell-detail">{item.message}</small></td><td>{item.email}</td><td>{item.service_name}</td><td>{item.city}</td><td><select className={`status-select ${item.status}`} value={item.status} onChange={async e=>{const status=e.target.value;update(item.id,{status});if(!supabase){const response=await fetch("/api/admin/data",{method:"PATCH",headers:{"Content-Type":"application/json"},body:JSON.stringify({section:"quotes",id:item.id,status})});if(!response.ok){toast.error("No fue posible actualizar el estado");return;}}toast.success("Estado actualizado");}}><option value="new">Nueva</option><option value="reviewed">Revisada</option><option value="contacted">Contactada</option><option value="closed">Cerrada</option></select></td></tr>)}</tbody></table></div></section>}
    {section === "images" && <section className="admin-card"><label className="upload-zone"><Upload /><strong>Subir una imagen</strong><span>PNG, JPG o WebP · hasta 4 MB</span><input type="file" accept="image/png,image/jpeg,image/webp" onChange={e=>uploadImage(e.target.files?.[0])}/></label><div className="image-admin-grid">{items.length===0?<div className="empty-state"><ImageIcon/><strong>Aún no hay imágenes</strong><span>Las imágenes cargadas aparecerán aquí.</span></div>:items.map(item=><article key={item.id}>{item.image_url?<img src={item.image_url} alt={item.title}/>:<div className="image-placeholder"><ImageIcon/></div>}<div><input value={item.title} onChange={e=>update(item.id,{title:e.target.value})}/><button onClick={()=>removeItem(item.id)}><Trash2/></button></div></article>)}</div></section>}
    {(section === "home" || section === "contact") && <section className="admin-card settings-form"><h2>{section === "home"?"Contenido principal":"Información de contacto"}</h2>{(section === "home" ? [["hero_title","Título principal"],["hero_text","Texto de portada"],["about_title","Título Sobre AUTOMEGA SpA"]] : [["phone","Teléfono"],["whatsapp","WhatsApp"],["email","Correo"],["address","Dirección"],["coverage","Cobertura"]]).map(([key,label])=><label key={key}>{label}{key.includes("text")?<textarea rows={4} value={settings[key as keyof typeof settings]} onChange={e=>setSettings({...settings,[key]:e.target.value})}/>:<input value={settings[key as keyof typeof settings]} onChange={e=>setSettings({...settings,[key]:e.target.value})}/>}</label>)}{section === "home" && <div className="upload-pair"><label className="upload-inline">Cambiar logo<input type="file" accept="image/*" onChange={e=>uploadImage(e.target.files?.[0],"logo")}/></label><label className="upload-inline">Cambiar imagen principal<input type="file" accept="image/*" onChange={e=>uploadImage(e.target.files?.[0],"hero")}/></label><label className="upload-inline">Cambiar imagen “Sobre AUTOMEGA SpA”<input type="file" accept="image/*" onChange={e=>uploadImage(e.target.files?.[0],"about")}/></label></div>}<button className="button button-yellow" onClick={saveSettings}><Save/> Guardar cambios</button></section>}
