...
export default function AdminWorkspace({ section }: { section: string }) {
  ...
  const [settings, setSettings] = useState({ hero_title:"Seguridad y control en cada vía", hero_text:"Servicios de conificación y señalización vial temporal para obras, faenas y desvíos.", about_title:"Seguridad vial para cada trabajo", phone:"+56 9 XXXX XXXX", whatsapp:"+56 9 XXXX XXXX", email:"contacto@automega.cl", address:"Concepción, Región del Biobío", coverage:"Concepción y toda la Región del Biobío", site_name:"AUTOMEGA" });
  ...
  async function saveItems() {
    ...
    setLoading(false); toast.success(hasSupabaseConfig ? "Cambios guardados en Supabase" : "Cambios guardados en modo demostración");
  }
  ...
  return <>
    ...
    {!hasSupabaseConfig && <div className="demo-banner">Modo demostración: conecta Supabase para guardar los cambios de forma permanente.</div>}
    {loading && <div className="admin-loading">Actualizando información…</div>}
    {(section === "services" || section === "projects") && <section className="admin-card editor-card"><div className="editor-toolbar"><h2>{section === "services" ? "Servicios publicados" : "Proyectos y trabajos"}</h2><button className="button button-yellow" onClick={addItem}><Plus /> Agregar</button></div><div className="editor-list">{items.map(item=><article className="editor-row" key={item.id}><div className="order-buttons"><button onClick={()=>move(item.id,-1)} aria-label="Subir"><ArrowUp /></button><button onClick={()=>move(item.id,1)} aria-label="Bajar"><ArrowDown /></button></div><div className="editor-fields"><input value={item.title} onChange={e=>update(item.id,{title:e.target.value})}/><textarea value={item.description} onChange={e=>update(item.id,{description:e.target.value})}/>{section === "projects" && <input placeholder="Ubicación" value={item.location||""} onChange={e=>update(item.id,{location:e.target.value})}/>}</div><button className={`visibility ${item.visible?"on":""}`} onClick={()=>update(item.id,{visible:!item.visible})}>{item.visible?<Eye/>:<EyeOff/>}{item.visible?"Publicado":"Oculto"}</button><button className="icon-button danger" onClick={()=>removeItem(item.id)}><Trash2 /></button></article>)}</div><button className="button button-yellow save-bottom" onClick={saveItems}><Save /> {loading?"Guardando…":"Guardar cambios"}</button></section>}
