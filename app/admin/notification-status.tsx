...
export default function NotificationStatus(){
  ...
  if(!data)return null;
  return <div className="notification-status"><div><b>Notificaciones por correo · {data.configured?'Variables presentes':'Pendiente de configurar'}</b><small>{data.counts.sent||0} aceptadas por el proveedor · {data.counts.pending||0} pendientes · {data.counts.failed||0} con error</small>{data.configured && <small>Las variables están presentes, pero la entrega depende de la validación de Resend.</small>}{!data.configured && <small>La solicitud se guarda aunque falte activar el proveedor de correo.</small>}{lastError && <small className="notification-error-detail">Resend: {lastError}</small>}</div>{data.configured && <button className="button button-white" onClick={retry}>Procesar pendientes</button>}</div>;
}
