...
export default function AdminDashboard() {
  ...
  return <>
    <section className="dashboard-hero"><div><span className="dashboard-kicker">CENTRO DE CONTROL · AUTOMEGA SpA</span><h1>Resumen operativo</h1><p>Supervisa solicitudes, proyectos y contenido publicado desde un solo lugar.</p><div className="dashboard-hero-meta"><span>Gestión de contenido</span><span>Acceso administrativo</span></div></div><div className="live-badge"><span />{loading ? 'Consultando…' : error ? 'Actualización pendiente' : 'Datos actualizados'}<small>{updatedAt && `Última consulta: ${updatedAt}`}</small><button className="dashboard-refresh" onClick={() => void refresh()} disabled={loading}>Actualizar</button></div></section>
