...
export default function AdminDashboard() {
  ...
  useEffect(()=>{fetch("/api/admin/summary",{cache:"no-store"}).then(r=>r.ok?r.json():null).then(value=>{if(value)setData(value);}).finally(()=>setLoading(false));},[]);
  ...
  return <>
    ...
    <section className="stats-grid">{[[FileText,"Solicitudes nuevas",counts.newQuotes,"Nuevas solicitudes de cotización"],[FolderKanban,"Proyectos publicados",counts.projects,"Proyectos visibles en el sitio"],[HardHat,"Servicios activos",counts.services,"Servicios publicados"],[ImageIcon,"Imágenes",counts.images,"Imágenes en la galería"]].map(([Icon,label,value,detail])=><article className="stat-card" key={String(label)}><span><Icon /></span><div><b>{String(label)}</b><strong>{loading?"—":String(value).padStart(2,"0")}</strong><p>{String(detail)}</p></div></article>)}</section>
